...
class ToggleSettingFlagTests(TestCase):
    def setUp(self):
        ...
        self.mgr = User.objects.create_user(username='flag_mgr', password='x', role='MANAGER')
        self.agent = User.objects.create_user(username='flag_agent', password='x', role='AGENT')
        self.web = Client()
    ...
    def test_agent_is_forbidden(self):
        self.web.force_login(self.agent)
        resp = self._post({'flag': 'email_sweep_autorun', 'enabled': True})
