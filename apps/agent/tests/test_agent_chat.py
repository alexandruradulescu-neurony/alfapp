...
class TestBuildPrompt:
    ...
    def test_build_prompt_with_claim_context(self, agent_chat_service):
        """Test building prompt with claim context."""
        context = {
            "claims": [
                {
        ...
        prompt = agent_chat_service.build_prompt("What is the status?", context)

        assert "ALF1234567" in prompt
        assert "test@example.com" in prompt
        assert "Received" in prompt
        assert "Black suitcase" in prompt

    def test_build_prompt_with_conversation_history(self, agent_chat_service):
        ...
        prompt = agent_chat_service.build_prompt("How are you?", context, conversation_history)

        assert "Previous conversation:" in prompt
        assert "Hello" in prompt
        assert "Hi there!" in prompt

    def test_build_prompt_no_claim_context(self, agent_chat_service):
        """Test building prompt without claim context."""
        context = {
            "claims": [],
            "emails": {},
            "refunds": {},
        ...
        prompt = agent_chat_service.build_prompt("General question", context)

        assert "No claim data available" in prompt or "Ask the user" in prompt

    def test_build_prompt_with_emails(self, agent_chat_service):
        """Test building prompt with email context."""
        context = {
            "claims": [],
            "emails": {
                "ALF1234567": [
                    {
                        "subject": "Test email",
                        "received_at": "March 18, 2026",
                        "ai_summary": "Summary",
                        "category": "OBJECT_FOUND",
                        "action_required": "Yes",
                        "body": "Email body content " * 100,  # Long body
                    }
                ]
            },
            "refunds": {},
        ...
        prompt = agent_chat_service.build_prompt("Show emails", context)

        assert "Test email" in prompt
        assert "OBJECT_FOUND" in prompt
        # Long body should be truncated
        assert len(prompt) < 10000

    def test_build_prompt_with_refunds(self, agent_chat_service):
        """Test building prompt with refund context."""
        context = {
            "claims": [],
            "emails": {},
            "refunds": {
        ...
        prompt = agent_chat_service.build_prompt("Show refunds", context)

        assert "REFUND:" in prompt
        assert "USD 100.00" in prompt
...
class TestCallLLM:
    ...
    @patch("openai.OpenAI")
    def test_successful_llm_call(self, mock_openai_class, mock_settings_class, agent_chat_service):
        ...
        result = agent_chat_service._call_llm("Test prompt")

        assert result == "This is the AI response"
        mock_client.chat.completions.create.assert_called_once()

    @patch("apps.config.models.SystemSettings")
    def test_llm_not_configured(self, mock_settings_class, agent_chat_service):
        """Test when AI is not configured."""
        mock_settings = Mock()
        ...
        assert "AI Not Configured" in result
        assert "Manager → Configuration" in result

    @patch("apps.config.models.SystemSettings")
    @patch("openai.OpenAI")
    def test_llm_api_error(self, mock_openai_class, mock_settings_class, agent_chat_service):
        """Test LLM API error."""
        mock_settings = Mock()
        mock_settings.ai_api_key = "test_key"
        mock_settings.ai_api_base = "https://api.test.com/v1"
    ...
    @patch("apps.config.models.SystemSettings")
    @patch("openai.OpenAI")
    def test_llm_returns_json(self, mock_openai_class, mock_settings_class, agent_chat_service):
        """Test handling when LLM returns JSON instead of natural language."""
        mock_settings = Mock()
        mock_settings.ai_api_key = "test_key"
        mock_settings_class.get_instance.return_value = mock_settings
