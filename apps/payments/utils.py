...
def generate_proof_of_work_pdf(claim: Claim) -> Optional[bytes]:
    ...
    Steps:
    a) Fetch claim.evidence.all() — all ClaimEvidence images
    b) Call fetch_zendesk_comments(claim.zd_ticket_id) from integrations service
    c) For each evidence image: open file, base64 encode, create data URI string
    d) Render template with claim data, comments, and images
    e) Convert rendered HTML to PDF using WeasyPrint
    f) Return PDF as bytes
    ...
    try:
        # a) Fetch all evidence images
        evidence_list = []
        for evidence in claim.evidence.all():
            try:
                # c) Open file and create base64 data URI
                if evidence.image:
                    # SECURITY: Validate file path is within MEDIA_ROOT (prevent path traversal)
                    abs_path = os.path.abspath(evidence.image.path)
                    media_root = os.path.abspath(settings.MEDIA_ROOT)
                    
                    if not abs_path.startswith(media_root):
                        logger.warning(
                            f"Evidence file path outside MEDIA_ROOT: {abs_path}"
                        )
                        continue
